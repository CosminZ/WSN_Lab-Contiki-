...
\DESCRIPTION:   
       - This File is part of the wsnlab project
       - It reads/manipulate platypus sensors and send over network
...
//#define serverIPv6 "bbbb::fec2:deff:fe38:7234"
...
bool getSensorsID();	//Get sensors ID
void Initializations();	//Setup IMU, battery gauge, LDC
void BatteryGauge();  	//Battery gauge
void readIMU();		//IMU
void getEnvSensors();	//Envornmental Sensors
void printValues();	//Print Accelerometer
...
{		
	cout << "[IMU] ";
	if (!printSensorsID(m_imu->getID(), m_imuID))
		success = false;
	
	cout << "[BAT] ";
	if (!printSensorsID(m_batgauge->getVersion(), m_batID))
		success = false;	

	cout << "[MAG] ";
	if (!printSensorsID(m_imu->getMagID(), m_magID))	
		success = false;
	
	cout << "[ENV] ";
	if (!printSensorsID(m_imu->getEnvID(), m_envID))
		success = false;	
...
void BatteryGauge()
{
	...
	{	
		//cout << "\n";
		cout << "Battery Voltage: " << m_batgauge->getVCell() << "V" << endl;
		cout << "State of charge: " << m_batgauge->getSoC() << "%" << endl;
		cout << "Alert Threshold: "<< m_batgauge->getAlertThreshold() << "%" << endl;
...
{	
	...
	cout << "\r"
	     << setw(12) << data[6] <<"\t"
	     << setw(10) << data[0] << setw(10) << data[1] << setw(10) << data[2] <<"\t"
	     << setw(10) << data[3] << setw(10) << data[4] << setw(10) << data[5] <<"\t"
	     << setw(10) << mx      << setw(10) << my      << setw(10) << mz << std::flush; 
}
