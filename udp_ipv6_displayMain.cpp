...
#define PORT_SEND	8880 //port from which to send data as acknowledgement
...
Graphics_Display info_display; // size of display
...
void initialize(); // instead create new object
...
int main(int argc, char** argv) {
	...
	display_edison* m_dsp;// a pointer to display structure
	...
	if (m_start_dsp)                                                                                 
           m_dsp = new display_edison(DISPSIZE);       // create new object and call defualt constructors which initialize the display

        uint16_t h = Graphics_getDisplayHeight(&g_sContext); // get height of the disply
        uint16_t w = Graphics_getDisplayWidth(&g_sContext);  // get width of the disply
	...
	int response_figure; // ddeclaration of variable
